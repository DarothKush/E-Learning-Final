    ...
    public function cart()
{
    ...
    public function cartItem(){
        $user = Auth::user();
        $userId= $user->registerUser_id;
        $cart = Cart::where('user_id', $userId)
        ->join('courses', 'cart.course_id', '=', 'courses.course_id')
        ->select('courses.course_name', 'courses.course_image', 'courses.price')
        ->get();
        $totalPrice = 0;
        foreach($cart as $i){
            $totalPrice += $i->price;
        }
        return response()->json([$cart, ['Total Price'=> $totalPrice]]); 
    }
}
